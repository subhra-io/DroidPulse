...
const initSchema = (db) => {
  db.exec(`
    ...
    -- Users: team members who access the dashboard
    CREATE TABLE IF NOT EXISTS users (
      ...
      project_id   TEXT,          -- NULL means access to all projects (super_admin)
  ...
  if (!adminExists) {
    ...
    db.prepare(`
      INSERT INTO users (id, email, name, password_hash, role, project_id)
      VALUES (?, ?, ?, ?, ?, NULL)
    `).run('user-admin-001', 'admin@droidpulse.dev', 'Super Admin', hash, 'super_admin')
    console.log('✅ Default super_admin created — email: admin@droidpulse.dev  password: admin123')
